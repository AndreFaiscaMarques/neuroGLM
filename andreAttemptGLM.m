...
load('C:\Users\ChiappeLab\Dropbox (Sensorimotor)\ChiappeLabNew\data\ANDRE\Calcium Imaging\Test Images\Wynne\R11R12 (ring)\Pooled Angular Trials\Behavior_All_Trials\190523_01_10_TM16_20.mat')
load('C:\Users\ChiappeLab\Dropbox (Sensorimotor)\ChiappeLabNew\data\ANDRE\Calcium Imaging\Test Images\Wynne\R11R12 (ring)\Pooled Angular Trials\All_Trials\190523\190523_01_10_trial16_20deltaF_ROIs_1.mat')

dataTogether = reshape(data, size(data,1),size(data,2)*size(data,3));
TMTogether = reshape(TM_DataAll, 3,size(TM_DataAll,2)*size(TM_DataAll,3));
...
expt = buildGLM.initExperiment(unitOfTime, binSize, [], FrameRate);
expt = buildGLM.registerContinuous(expt, 'forw', 'forward', 1); % continuous obsevation over time
expt = buildGLM.registerContinuous(expt, 'ang', 'angular', 1); 
expt = buildGLM.registerContinuous(expt, 'sid', 'side', 1);
...
expt = buildGLM.registerSpikeTrain(expt, 'voltage', 'Our Neuron'); % Spike train!!!
...
dspec = buildGLM.addCovariateRaw(dspec, 'forw', [], bs);

%% Spike history
dspec = buildGLM.addCovariateSpiketrain(dspec, 'hist', 'voltage', 'History filter');
